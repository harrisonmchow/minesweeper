...
#
#
# This program was written by Harrison Chow (z5366469)
# on 17th October 2021, finished on 30th October 2021. 
#
...
reveal_grid__body:
        ...
        li      $t0, 0              # int row = 0;
reveal_row_loop:     
        bge     $t0, N_ROWS, reveal_grid__epilogue
        # int col = 0;
        li      $t1, 0              # int col = 0;
reveal_col_loop: 
        bge     $t1, N_COLS, after_reveal_col
        
        mul     $t2, $t0, 1         #calculate address of grid[row][col]
        mul     $t2, $t2, N_COLS
        mul     $t3, $t1, 1
        la      $t4, grid
        add     $t5, $t4, $t2
...
place_bombs_loop: 
        bge     $s4, $s5, place_bombs__epilogue
        addiu   $sp, $sp, -8
        sw      $a0, 4($sp)
        sw      $a1, ($sp)
...
mark_cell_revealed: 
        ...
        mul     $t0, $a2, N_COLS    # calculate address of grid[row][col]
        la      $t2, grid
        add     $t3, $t2, $t0
        add     $t3, $t3, $a1
        lb      $t4, ($t3)          # address of grid in $t3, grid[row][col] in $t4
        
        andi    $t5, $t4, IS_RVLD_MASK
        beqz    $t5, mark_cell_unmarked
        lw      $t1, debug_mode
        beq     $t1, 1, mark_cell_debug
        
        la      $a0, mark_error     # printf
        li      $v0, 4
        syscall
...
mark_cell_unmarked:
        ...
        beqz    $t6, mark_cell_unmarked_else
        li      $t1, IS_MRKD_MASK    
        not     $t7, $t1
        and     $t4, $t4, $t7
        sb      $t4, ($t3)
        ...
        la      $t9, bomb_count
        lw      $t8, ($t9)
        addi    $t8, $t8, 1         # bomb_count++;
        sw      $t8, ($t9)
        
        j       mark_cell__epilogue
mark_cell_unmarked_else: 
        or      $t4, $t4, IS_MRKD_MASK
        sb      $t4, ($t3)
...
reveal_cell__body:
        ...
        lb      $t4, ($t3)          # address of grid in $t3, grid[row][col] in $t4
reveal_cell_is_bomb:        
        and     $t5, $t4, IS_BOMB_MASK
        beqz    $t5, reveal_cell_not_bomb #if $t5 is 0, go to reveal_cell_not_bomb
        li      $t6, LOSE           #if you reveal a bomb, game_state = LOSE
        la      $t7, game_state
        sw      $t6, ($t7)
        j       reveal_cell_valid
reveal_cell_not_bomb:         
        and     $t7, $t4, IS_MRKD_MASK
        beqz    $t7, reveal_cell_not_marked
        lw      $t8, debug_mode
        beqz    $t8, reveal_cell_marked_not_debug   #if debug mode is on, return
        j       reveal_cell_win_check
reveal_cell_marked_not_debug:
        la      $a0, reveal_error     # printf
        li      $v0, 4
        syscall
        j       reveal_cell_win_check
reveal_cell_not_marked:
        and     $t9, $t4, IS_RVLD_MASK
        beqz    $t9, reveal_cell_valid
        lw      $t8, debug_mode       #if debug mode, just return.
        beqz    $t8, reveal_cell_already_revealed
        j       reveal_cell_win_check
reveal_cell_already_revealed:
        la      $a0, already_revealed     # printf
        li      $v0, 4
        syscall
        j       reveal_cell_win_check
reveal_cell_valid:
        and     $t5, $t4, VALUE_MASK
        bnez    $t5, reveal_cell_not_recursive
        ...
        j       reveal_cell_win_check
reveal_cell_not_recursive:
        or      $t4, $t4, IS_RVLD_MASK
        sb      $t4, ($t3)
        
        li      $t6, LOSE
        la      $t9, game_state
        lw      $t7, ($t9)
        beq     $t7, $t6, reveal_cell_win_check
        
        la      $t9, cells_left
        lw      $t8, ($t9)
        addi    $t8, $t8, -1        #cells_left--
        sw      $t8, ($t9)
reveal_cell_win_check:
        la      $t9, cells_left
        lw      $t8, ($t9)
        bnez    $t8, reveal_cell__epilogue  #if cells_left == 0, game_state = 0;
        li      $t9, WIN
        la      $t7, game_state
        sw      $t9, ($t7)  
...
clear_surroundings__body:
        ...
        blt     $a0, 0, clear_surroundings__epilogue    #if row < 0, return
        bge     $a0, N_ROWS, clear_surroundings__epilogue   #if row >= N_ROWS, return
        blt     $a1, 0, clear_surroundings__epilogue    #if col < 0, return
        bge     $a1, N_COLS, clear_surroundings__epilogue   #if col >= N_ROWS, return
        
        mul     $t0, $a0, N_COLS    #calculate address of grid[row][col]
        la      $t2, grid
        add     $t3, $t2, $t0
        add     $t3, $t3, $a1
        lb      $t4, ($t3)
        
        and     $t5, $t4, IS_RVLD_MASK
        bnez    $t5, clear_surroundings__epilogue 
        
        or      $t4, $t4, IS_RVLD_MASK  #grid[row][col] |= IS_RVLD_MASK;
        sb      $t4, ($t3)
        
        la      $t9, cells_left
        lw      $t6, ($t9)
        addi    $t6, $t6, -1        #cells_left--;
        sw      $t6, ($t9)
        
        li      $t7, IS_MRKD_MASK
        not     $t7, $t7            #grid[row][col] &= ~IS_MRKD_MASK;
        and     $t4, $t4, $t7 
        sb      $t4, ($t3)
        
        and     $t8, $t4, VALUE_MASK
        bnez    $t8, clear_surroundings__epilogue
        
clear_surroundings_recursive:
        #   clear_surroundings(row - 1, col);
...
update_highscore:
        ...
        #       -> highscore loop 
        #       -> return TRUE
        #       -> return FALSE if dont need to update highscore
        #   -> [epilogue]
...
update_highscore__body:
        ...
        la      $t0, high_score
        lw      $t1, ($t0)          # $t1 = high_score.score
        bge     $t1, $a0, update_highscore_false
        
        move    $t1, $a0            # high_score.score = score;
        sw      $t1, ($t0)
        
        li      $t2, 0              # int i = 0;
        li      $t3, 0
 
update_highscore_loop:
        
        la      $t4, user_name
        add     $t4, $t4, $t2
        lb      $t5, ($t4)
        
        beq     $t5, $t3, update_highscore_true_after_loop
        
        la      $t0, high_score     # go to address holding the name
        addi    $t0, $t0, N_INT_BYTES
        add     $t0, $t0, $t2
        ...
        addi    $t2, $t2, 1         # i++;
        j       update_highscore_loop
        
update_highscore_true_after_loop:
        la      $t4, user_name      # high_score.name[i] = '\0';
        add     $t4, $t4, $t2
        lb      $t5, ($t4)
...
print_scores__body:
        ...
        li      $t0, 0              # int i = 0;
print_scores_loop:
        bge     $t0, MAX_SCORES, print_scores_after
        
        la      $t1, scores         # get address of scores[i]
        mul     $t2, $t0, USER_SCORE_SIZE
        add     $t3, $t1, $t2
        
        lw      $t4, ($t3)          # $t4 = curr.score        
        li      $t5, -1
        beq     $t4, $t5, print_scores__epilogue
        ...
        li      $t6, 0
        #li      $t7, 0              # j = 0
        addi    $t3, $t3, N_INT_BYTES   # go to address holding the name
print_names_loop:
                    
        lb      $t8, ($t3)
        
        beq     $t8, $t6, print_scores_after
        
        move    $a0, $t8            # printf("%c");
        li      $v0, 11
        syscall 
        
        addi    $t3, $t3, 1         # go to the character
        j       print_names_loop
...
print_scores_after:
        ...
        move    $a0, $t4            # printf("%d", curr.score")
        li      $v0, 1
        syscall 
